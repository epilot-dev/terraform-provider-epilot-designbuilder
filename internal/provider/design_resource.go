			...
			"design_tokens": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					...
					"card_variant": schema.StringAttribute{
						Computed:    true,
						Optional:    true,
						Description: `Card visual variant. must be one of ["shadow", "outlined"]`,
					...
					"font_size_scale": schema.StringAttribute{
						Computed:    true,
						Optional:    true,
						Description: `Font size scale factor. must be one of ["xs", "sm", "md", "lg", "xl"]`,
						Validators: []validator.String{
							stringvalidator.OneOf(
								"xs",
								"sm",
					...
					"input_variant": schema.StringAttribute{
						Computed:    true,
						Optional:    true,
						Description: `Input field variant style. must be one of ["outlined", "filled", "underlined"]`,
						Validators: []validator.String{
							stringvalidator.OneOf(
								"outlined",
								"filled",
					...
					"topbar_logo_alignment": schema.StringAttribute{
						Computed:    true,
						Optional:    true,
						Description: `Logo/content alignment in the top bar. must be one of ["flex-start", "center", "flex-end"]`,
						Validators: []validator.String{
							stringvalidator.OneOf(
								"flex-start",
								"center",
...
func (r *DesignResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	request1, request1Diags := data.ToOperationsGetDesignRequest(ctx)
	resp.Diagnostics.Append(request1Diags...)

	if resp.Diagnostics.HasError() {
		return
	}
	res1, err := r.client.DesignBuilder.GetDesign(ctx, *request1)
	if err != nil {
		resp.Diagnostics.AddError("failure to invoke API", err.Error())
	...
	resp.Diagnostics.Append(data.RefreshFromSharedGetDesignRes(ctx, res1.GetDesignRes)...)
